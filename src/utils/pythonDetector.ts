...
export class PythonDetector {
  ...
  static async detectPythonInterpreters(): Promise<PythonInterpreter[]> {
    ...
    const uniqueInterpreters = interpreters.filter(
      (interpreter, index, self) =>
    ...
    uniqueInterpreters.sort(
      (a, b) => sortOrder[a.source] - sortOrder[b.source],
    );
    ...
    try {
      ...
      if (pythonExtension && pythonExtension.isActive) {
        ...
        if (
          ...
          if (
            execDetails &&
            execDetails.execCommand &&
            execDetails.execCommand.length > 0
          ) {
   ...
   */
  private static async detectWorkspaceInterpreters(): Promise<
    PythonInterpreter[]
  > {
    const interpreters: PythonInterpreter[] = [];
      ...
      for (const pythonPath of venvPaths) {
        if (await this.fileExists(pythonPath)) {
          ...
          interpreters.push({
            path: pythonPath,
            version,
            displayName: `Python ${version || "Unknown"} (${path.basename(
   ...
   */
  private static async detectSystemInterpreters(): Promise<
    PythonInterpreter[]
  > {
    const interpreters: PythonInterpreter[] = [];
   ...
   */
  static async showPythonInterpreterPicker(): Promise<
  ...
  > {
    ...
    if (interpreters.length === 0) {
      vscode.window.showErrorMessage(
        "No Python interpreters found. Please install Python or check your PATH.",
      );
      return undefined;
    }
  ...
  /**
   * Subscribes to Python interpreter changes from the Python extension if available.
   * Falls back to configuration change event for 'python' section.
   */
  static onPythonInterpreterChanged(handler: () => void): vscode.Disposable {
    try {
      const pythonExtension =
        vscode.extensions.getExtension("ms-python.python");
      if (pythonExtension && pythonExtension.isActive) {
        const api = pythonExtension.exports;
        if (api?.settings?.onDidChangeExecutionDetails) {
          return api.settings.onDidChangeExecutionDetails(handler);
        }
      }
    } catch {
      // ignore
   ...
   */
  private static async getPythonVersion(
    pythonPath: string,
