...
export class ConfigurationManager {
  ...
  /**
   * Sync alembic.pythonPath with Python extension's interpreter when it changes
   */
  static subscribePythonInterpreterSync(): vscode.Disposable {
    const disposer = PythonDetector.onPythonInterpreterChanged(async () => {
   ...
   */
  static affectsAlembicConfiguration(
  ...
  static async selectPythonInterpreter(): Promise<void> {
    ...
    if (interpreter) {
      ...
      vscode.window.showInformationMessage(
        `Python interpreter set to: ${interpreter.displayName}`,
      );
    }
  }
