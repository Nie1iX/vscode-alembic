...
export class AlembicService {
	...
	async initAlembic(): Promise<void> {
		...
		try {
			const command = this.buildCommand(['init', '--template', template, directoryName]);
			await this.executeCommand(command, workspaceFolder);
			vscode.window.showInformationMessage('Alembic initialized successfully!');
			vscode.commands.executeCommand('setContext', 'alembic:hasAlembicConfig', true);
		} catch (error) {
	...
	async createMigration(): Promise<void> {
		...
		try {
			const command = this.buildCommand(['revision', '--autogenerate', '-m', message]);
			await this.executeCommand(command);
			vscode.window.showInformationMessage(`Migration "${message}" created successfully!`);
	...
	async getMigrations(): Promise<Migration[]> {
		try {
			const historyResult = await this.executeCommand(this.buildCommand(['history']));
			const currentResult = await this.executeCommand(this.buildCommand(['current']));

			return this.parseMigrations(historyResult, currentResult);
		} catch (error) {
	...
	async getMigrationGraph(): Promise<{ nodes: any[], edges: any[] }> {
		try {
			...
			const nodes = migrations.map(migration => ({
				...
				color: migration.isCurrent ? '#4CAF50' : migration.isApplied ? '#2196F3' : '#FFC107',
				font: { color: 'white' }
			}));
	...
	private parseMigrations(historyOutput: string, currentOutput: string): Migration[] {
		...
		for (const line of lines) {
			const match = line.match(/^([a-f0-9]+)\s+\->\s+([a-f0-9]+)?\s*,?\s*(.*)$/);
			if (match) {
	...
	private async getAvailableTemplates(): Promise<Array<{label: string, description: string, detail?: string}>> {
		try {
			const command = this.buildCommand(['list_templates']);
			this.outputChannel.appendLine('Getting available templates from Alembic...');
			const output = await this.executeCommand(command);

			const templates: Array<{label: string, description: string, detail?: string}> = [];
			...
			for (const line of lines) {
				...
				if (inTemplateSection && (line.includes('Templates are used via') || line.includes('alembic init'))) {
					break;
				}
			...
			if (templates.length > 0) {
				...
				this.outputChannel.appendLine('No templates found in Alembic output, using fallback');
			}
		} catch (error) {
			this.outputChannel.appendLine(`Warning: Could not get templates from Alembic: ${error}`);
		}
	...
	private getTemplateDetail(label: string): string | undefined {
		const templateDetails: {[key: string]: string} = {
			...
			'async': 'For projects using async/await database operations (asyncio, asyncpg, etc.)',
			...
			'pyproject_async': 'Combines pyproject.toml configuration with async database operations'
		};

		return templateDetails[label];
	}

	private getFallbackTemplates(): Array<{label: string, description: string, detail?: string}> {
